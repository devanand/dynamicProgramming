...
public class SkiingChallenge {
	...
	static int longestDecreasingDepth(int mat[][], int memo[][], int i, int j, int x, int y, long sumMemo[][]) {

		//int maxPath=0;
		if(memo[i][j] != 0) {
			long temp = (mat[x][y] - mat[i][j])+sumMemo[i][j];
			if(temp>sumMemo[x][y]) {
				 sumMemo[x][y]=temp;
			}
			return memo[i][j];
	...
	public static void main(String[] args) throws Exception {
		...
		long sumMemo[][] = new long[mat.length][mat[0].length];
